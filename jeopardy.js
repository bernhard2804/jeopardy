...
function writeAllToHTML(masterarray){
    console.log('inside writeAllToHTML')
    $('#gameboard').empty();
    console.log($('#gameboard'));
    writeTableHead(masterarray);
}
...
async function getClues(indexArr){          // indexArr von 6 [{title, categroyId}, ...] wird übergeben
    ...
    indexArr.forEach(async function(val){       // val ist ein einzelnes Object {title, categoryId}
        ...
        newClues = await getCluesFromOneCat(val);
        newArray.push(val);
            });
...
async function setupAndStart() {
    ...
    const masterarray = await getClues(indexArr);
    console.log('Das MasterArray: ', masterarray);
    writeAllToHTML(masterarray);
}
